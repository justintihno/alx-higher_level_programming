def matrix_mul(m_a, m_b):
    ...
    if any(not isinstance(num, (int, float)) for row in m_a for num in row) or \
       any(not isinstance(num, (int, float)) for row in m_b for num in row):
        raise TypeError("m_a should contain only integers or floats" if any(not isinstance(num, (int, float)) for row in m_a for num in row) else "m_b should contain only integers or floats")

    if any(len(row) != len(m_a[0]) for row in m_a) or any(len(row) != len(m_b[0]) for row in m_b):
        raise TypeError("each row of m_a must be of the same size" if any(len(row) != len(m_a[0]) for row in m_a) else "each row of m_b must be of the same size")
